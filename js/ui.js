...
export function renderToolbar() {
    ...
    if (ngEl) ngEl.textContent = `${gs.naegong}/${gs.maxNaegong}`;
    if (ngFillEl) {
        const pct = gs.maxNaegong > 0 ? (gs.naegong / gs.maxNaegong) * 100 : 0;
...
function renderStats() {
    ...
    const cards = [
        ...
        { label: '내공', value: `${gs.naegong}/${gs.maxNaegong}`, icon: 'fa-fire', color: 'text-orange-400' },
    ];
...
export function renderMainPanel() {
    ...
    panel.innerHTML = `
        <div class="flex items-center gap-4 mb-6">
            ...
            <div class="flex-1">
                <h2 class="text-2xl font-bold text-amber-300">방랑 협객</h2>
                <p class="text-zinc-400">Lv.${gs.level} · 공격 ${gs.atk} · 방어 ${gs.def}</p>
        ...
        <div class="grid grid-cols-3 gap-3 mb-6">
            ...
            <button onclick="window.gatherIntel()"
                ...
                <div class="text-xs font-normal text-blue-300/80 mt-1">성공률 높음 · 적 조우 34%</div>
            </button>
            <button onclick="window.openRestMenu()"
                ...
                <div class="text-xs font-normal text-emerald-300/80 mt-1">노숙 / 숙소 / 기연</div>
            </button>
        </div>
    ...
    return `
        <div class="flex gap-6">
            <div class="text-center shrink-0">
                <div class="sd-placeholder text-4xl mx-auto mb-3">🥷</div>
                <div class="font-bold text-lg text-amber-300">방랑 협객</div>
                <div class="${align.color} text-sm mt-1">${align.label}</div>
            </div>
            <div class="flex-1 space-y-4">
                ...
                <div class="grid grid-cols-3 gap-3 text-center text-sm">
                    <div class="bg-zinc-800/60 rounded-xl p-3"><div class="text-yellow-400 font-bold text-lg">${gs.fame}</div><div class="text-zinc-500">명성</div></div>
                    <div class="bg-zinc-800/60 rounded-xl p-3"><div class="text-red-400 font-bold text-lg">${gs.notoriety}</div><div class="text-zinc-500">악명</div></div>
                ...
                <div class="grid grid-cols-2 gap-2 text-sm">
                    ${[
                        ...
                        ['내공', `${gs.naegong}/${gs.maxNaegong}`, 'text-blue-400'],
...
function renderMurimPanel() {
    ...
    return `
        <div class="mb-4 p-4 bg-amber-900/20 border border-amber-800/50 rounded-xl">
            ...
            <p class="text-sm text-zinc-400">촉중 사천에서 여정을 시작했다. 검각관의 산적 소문이 강호에 퍼지고 있다.</p>
        </div>
